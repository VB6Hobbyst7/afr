...
Sub Class_Globals
	...
	Private phWakeLock As PhoneWakeState
...
Public Sub imgRoundedCorners(bm As Bitmap, view As ImageView)
...
Public Sub getConnectionType
	...
	'Log($"PHONE INFO ${phoneInfoText}"$)
	If mlWifi.isWifiConnected = True Then
		CallSub3(player, "setConnectionIcon", "WIFI connected"  & " (" & mlWifi.WifiIpAddress &", " & mlWifi.WifiLinkSpeed&" Mbps)", "wifi")
	...
	Else
		...
		End If
		CallSub3(player, "setWifiPhoneImage", Starter.vWifiConnected, phoneInfoText)
		CallSub3(player, "setConnectionIcon", phoneInfoText, "mobile")
	End If
