...
Private Sub sb_ValueChanged(value As Int, userchanged As Boolean)
	If value < 1 Then
		volImage.Bitmap	=  LoadBitmap(File.DirAssets,"outline_volume_off_black_24.png")' xml.GetDrawable("outline_volume_off_black_24")
	Else
		volImage.Bitmap	= LoadBitmap(File.DirAssets,"outline_volume_up_black_24.png")'xml.GetDrawable("outline_volume_up_black_24")
	End If
	
	volLabel.Text	= value
	ph.SetVolume(ph.VOLUME_MUSIC, value, False)
	SeekBarIndex = SeekBarIndex + 1
	Dim MyIndex As Int = SeekBarIndex
		
	Sleep(3000)
	If MyIndex <> SeekBarIndex Then Return
	pnlSlider.SetVisibleAnimated(3000, False)
End Sub
...
Private Sub createVolPanelImage As ImageView
	...
	volImage.Gravity = Gravity.FILL

	If getMusicVolume < 1 Then
		volImage.Bitmap	=  LoadBitmap(File.DirAssets,"outline_volume_off_black_24.png")
	Else
		volImage.Bitmap	= LoadBitmap(File.DirAssets,"outline_volume_up_black_24.png")
	End If
