...
Version=7.8
@EndOfDesignText@
'xCustomListView v1.02 - modified version (Expandable items)_
...
Sub Class_Globals
	...
	Type CLVItemExp(Panel As B4XView, Height As Int, Value As Object, Color As Int, TextItem As Boolean)
...
#End If
	For Each it As CLVItemExp In items
...
Private Sub GetItem(Index As Int) As CLVItemExp
...
Public Sub RemoveAt(Index As Int)
	If getSize <= 1 Then
		...
		Return
	End If
	Dim RemoveItem As CLVItemExp = items.Get(Index)
	Dim p As B4XView
	For i = Index + 1 To items.Size - 1
		Dim item As CLVItemExp = items.Get(i)
		p = item.Panel
		p.Tag = i - 1
...
Public Sub InsertAtTextItem(Index As Int, Text As Object, Value As Object)
	...
	Dim item As CLVItemExp = GetItem(Index)
	item.TextItem = True
End Sub
...
Private Sub CleanAfterAnimation
	...
	For Each item As CLVItemExp In items
		item.Panel.Top = Top
