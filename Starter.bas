...
Sub Process_Globals
	Private sleepTimer As playerTimer
	...
	Private vSongLyric As String	= "noLyric"
	...
	'STRING
	Public dbL, Username, Password, activeActivity, playerUsed, lastfmapi As String
	...
	Public chartArtist, chartSong, streamLostInfo, vSpotError As String
	...
	Public chatDataLyric, lyricsOnDemand, pnl_store_song_button, lyricFound, albumArtFound, streamLost As Boolean = False
	'FLOAT
...
Sub Service_Create
	clsFunc.Initialize
'	sleepTimer.Initialize(5000, 3600000)
'	sleepTimer.enableSleepTimer(False)
...
'	sleepTimer.Interval = sleepTimerDuration
'	sleepTimer.enableSleepTimer(enable)
End Sub

Sub Service_Start (StartingIntent As Intent)
	Service.StartForeground(notifId, createNotif("Not streaming.."))
	clsFunc.showLog(StartingIntent.Action, 0)
	...
	Try
		...
		If MethodName = "playerMetadata" Then
			...
			If Args(0) <> Null And Args(1) <> Null Then
				...
				If Args(0) = "icy-url" Then
					...
					If IsPaused(player) = False Then
						clsFunc.showLog("STATION URL : " & Args(1), 0)
						vStationUrl = Args(1)
						clsFunc.showLog("GETSTATIONLOGO : " & Args(1), 0)
			...
			Try
						...
						If lastSong = "" Or lastSong <> vSong Then
							...
							If activeActivity = "player" Then
								...
								Try
									...
									CallSubDelayed2(player, "enableAlbumInfo", False)
									If albumArtFound Then
										CallSub2(player, "enableAlbumButton", False)
									End If
									clsFunc.showLog($"Current time is $DateTime{DateTime.Now}"$, Colors.Red)
								Catch
				...
				End If
			Catch
				Log("HOE DAN " & LastException.Message)
			End Try
		End If
