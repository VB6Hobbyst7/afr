...
Sub getStationStreams(rs As JdbcResultSet) As String
...
Private Sub scrollTimer_Tick
	Starter.clsFunc.showLog("PPPP",0)
...
Sub Connect As ResumableSub
	...
	mysql.InitializeAsync("mysql", Starter.driver, Starter.dbL, Starter.Username, Starter.Password)
...
Sub getSearchStation(params As List) As ResumableSub
	...
	If Success Then
		Try
			...
			Dim sf As Object = mysql.ExecQueryAsync("mysql", "SELECT * FROM rdolist WHERE stname LIKE ? AND country = ? ORDER BY stname ASC", params)
...
Sub getStationStream(params As List) As ResumableSub
	Wait For (Connect) Complete (Success As Boolean)
	If Success Then
		Try
			Connect
	
			Dim sf As Object = mysql.ExecQueryAsync("mysql", "SELECT * FROM rdolist WHERE rdo_id = ?", params)
...
Sub lblstream_LongClick
	...
	getStationData(params, Starter.clsFunc.stringSplit(",", lbl.Tag, -1, False,0, False), stationId)
...
Sub addStation(stationId As String, stream As String)
	...
	getStationData(params, stream, panelIndex)
	Starter.stationAdded = 1
	
End Sub

Sub getStationData(params As List, stream As String, stationId As String) As ResumableSub
	Dim setStname, setDescription, setGenre, setCountry, setLanguage, lastid As String
	Dim list As List
	...
	If Success Then
		Try
			Connect
			Dim sf As Object = mysql.ExecQueryAsync("mysql", "SELECT stname, description, genre, country, language FROM rdolist WHERE rdo_id=?", params)
			Wait For (sf) mysql_QueryComplete (Success As Boolean, rs As JdbcResultSet)
			
			If Success Then
				list.Initialize
				Do While rs.NextRow
					setStname		= rs.GetString("stname")
					setDescription	= rs.GetString("description")
					setGenre		= rs.GetString("genre")
					setCountry		= rs.GetString("country")
					setLanguage		= rs.GetString("language")
					
					stationName	= setStname
				Loop
				lastid	= genDb.addStationToPreset(setStname, setDescription, setGenre, setCountry, setLanguage, stream, stationId)
				list.AddAll(Array As String(setStname, setGenre, stream, setDescription, lastid))
				mysql.Close
				
				ToastMessageShow(setStname & " added to presets", False)
				If showRestartInfo = 0 Then
					showRestartInfo = 1
				End If
			End If
		Catch
...
Sub getStationInfo(index As Int)
	...
	For Each v As B4XView In pnl.GetAllViewsRecursive
		If v Is Panel Then
			...
			If pnlTag.IndexOf("stationname-") > -1 Then
				...
				Starter.clsFunc.showLog(lbl_stationname.Text, Colors.Red)
				Exit
			End If
		End If
	Next
	...
	Wait For (CallSub2(Me, "getStationStream", param)) Complete (rs As JdbcResultSet)
...
Sub playSelectedStream(selectedStream As String)
	...
	Starter.clsFunc.showLog(selectedStream, 0)
...
Sub edt_find_EnterPressed
	...
	If vText.Length > 0 Then
		...
		Wait For (CallSub2(Me, "getSearchStation", params)) Complete (rs As JdbcResultSet)
