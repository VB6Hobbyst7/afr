...
Sub Globals
	...
	Private SwitchSleeptimer As ACSwitch
	...
	Private rsie As RSImageEffects
...
'	'***************************
	...
	Dim actionViewItem As ACMenuItem
	actionViewItem = NavDrawer.NavigationView.Menu.AddWithGroup2(1, 4, 2, "Sleep timer", xml.GetDrawable("ic_snooze_black_24dp"))
	SwitchSleeptimer.Initialize("SwitchSleeptimer")
	actionViewItem.ActionView = SwitchSleeptimer
	...
	clsScroll.runMarquee(lblArtistNowPlaying, lblNowPlayingStation.Text, "MARQUEE")
...
'
'Sub createPlayerStationBorder
'	Dim stationList As CustomListView = clvPlayer
'	Dim p As Panel
'	For i = 0 To stationList.Size -1
'		p = stationList.GetPanel(i)
'		drawPanelBorder(p, True)
'	Next
'End Sub
...
Sub restorePlayingInfo
	...
	Try
		...
		setPanelElevation(Starter.vPlayerSelectedPanel, True)
		...
		If Starter.vStationName <> "" And CallSub(Starter, "getSongPlaying") <> "" Then
			...
			imgRoundedCorners(LoadBitmap(File.DirAssets, "logo_afr.png"), ivNowPlaying)
		Else
			imgRoundedCorners(LoadBitmap(File.DirAssets, "logo_afr.png"), ivNowPlaying)
		End If
...
Sub Activity_Resume
	...
	If Starter.vPlayerSelectedPanel <> 999 Then
		...
		CallSubDelayed2(songdata,"spBearer", scrobbler.processPlaying(Starter.lastSong))
	Else
...
Sub Activity_Pause (UserClosed As Boolean)
	If UserClosed Then
		...
		resetKvsPnlButtons
		Starter.clsFunc.showLog("USERCLOSED",0)
		exitPlayer
	Else
		...
		Starter.clsFunc.showLog("BUTTON STATE getSetButtonState(True)", 0)
...
Sub setStation(lst As List) As Panel
	...
	Dim SVG As ioxSVG
	SVG.Initialize("013-play.svg")
	SVG.DocumentWidth = ImgSvg.Width
	SVG.DocumentHeight = ImgSvg.Height
	...
	If lst.get(6) <> Null And File.Exists("", lst.get(6))Then
		...
		imgRoundedCorners(bm, iv_station_logo)
...
Sub setAlbumArtFading(vArt As Bitmap)
	...
	imgRoundedCorners(vArt, ivNowPlaying)
End Sub


Sub restartStream
	If stream <> "" Then
		Starter.streamRestartCount = Starter.streamRestartCount + 1
	End If
End Sub
...
Sub streamPlaying(playing As Boolean)
	...
	If playing = False Then
		...
		If Starter.playerUsed	= "exo" Then
			lblPlayingButtonRef.SetBackgroundImage(LoadBitmap(File.DirAssets,"play.png"))
			lblPlayingButtonRef.Gravity = Gravity.FILL
			showSnackbar("Unable to play stream..")
...
'	clsFunc.showLog("PROCESSING PANEL " & Starter.vPlayerSelectedPanel, 0)
	For Each v As View In panel.GetAllViewsRecursive
		If v.Tag = "pnl_station_logo" Then
			pnl_logo = v
			ivLogo = pnl_logo.GetView(0)
			imgRoundedCorners(img, ivLogo)
			imgRoundedCorners(img, ivNowPlayingStation)
...
Sub setPanelElevation(index As Int, elevate As Boolean)
	...
	For Each v As View In pnl.GetAllViewsRecursive
		...
		If v.Tag Is List Then
			...
			If listPlayButton.Get(0)= "headerPlayButton" Then
				Starter.clsFunc.showLog( $"GENRE FROM LIST : ${listPlayButton.Get(2)}"$, 0)
...
Sub showOverflow(index As Int)
	Dim hideTmr As clsGenTimer

	hideTmr.Initialize(5000, "hideOverFlow")
	
	pnlOverflow.SetVisibleAnimated(0, True)
	pnlOverflow.SetLayoutAnimated(1000, pnlOverflow.Left, (Activity.Height- pnlOverflow.Height-0dip), pnlOverflow.Width, pnlOverflow.Height)
End Sub


Sub hideOverflow(index As Int)
...
Sub lblDeleteStation_Click
	...
	result 		= Msgbox2Async($"Delete ${stationName}"$, Starter.vAppname, "Yes", "", "No", Null, False)
...
Sub getStationLogo(link As String)
	...
	If genDb.checkStationLogo(stationId) = False Then
		If Starter.vUpdateLogo = False Or hasLogoPath.Length > 0  Then
			Starter.clsFunc.showLog("NO STATIONLOGO UPDATE", 0)
	...
	Starter.clsFunc.showLog("getStationLogo", 0)
	...
	Starter.clsFunc.showLog ($"${url}"$, 0)
	Dim j As HttpJob
	j.Initialize("", Me)
	j.Download(url)
	Wait For (j) JobDone(j As HttpJob)
	If j.Success Then
		Starter.clsFunc.showLog($"Station logo response ${j.Response}"$, 0)
...
Sub lblOpenStationUrl_Click
	...
	hideOverflow(lblOpenStationUrl.Tag)
End Sub
...
Sub lblHeaderPlayButton_Click
	Starter.clsFunc.showLog($"LBLHEADERPLAYBUTTON SENDER ${Sender}"$, 0)
	startOrStopStream(-1)
End Sub
...
Sub getSetSettings
	...
	If kvs.getSimple("sleep_timer") = 1 Then
		Starter.sleepTimerDuration = kvs.GetSimple("sleep_timer_duration")
		SwitchSleeptimer.Checked = True
		CallSub2(Starter, "enableSleepTimer", True)
	Else
		SwitchSleeptimer.Checked = False	
		CallSub2(Starter, "enableSleepTimer", False)
	End If
...
Sub SwitchUpdateLogo_CheckedChange(Checked As Boolean)
	If kvs.ContainsKey("updatelogo") = True Then
		...
		Starter.clsFunc.showLog("updateLogo " & Checked, 0)
	End If
End Sub


Sub SwitchSleeptimer_CheckedChange(Checked As Boolean)
	Dim sleepEnable As Int
	...
	Else 
		sleepEnable = 0	
		Starter.sleepTimerDuration = 0
	End If
	
	kvs.PutSimple("sleep_timer", sleepEnable)
	
	ivTimer.SetVisibleAnimated(2000, Checked)
	CallSub2(Starter, "enableSleepTimer", Checked)
End Sub
...
Sub writeDataUsageToKvs(vDataUsage As Float)
	Dim kvsDataUsage, kvsNewDataUsage As Float
	
	kvsDataUsage	= kvs.GetSimple("data_usage")
	kvsNewDataUsage	= kvsDataUsage + vDataUsage'(kvsDataUsage-vDataUsage)'+vDataUsage

	kvs.PutSimple("data_usage",kvsNewDataUsage)
End Sub
...
Sub getSetButtonState(set As Boolean)
	...
	Else
		...
		imgRoundedCorners(bm, ivNowPlayingStation)
...
Sub imgRoundedCorners(bm As Bitmap, view As ImageView)
	...
	If bm.IsInitialized = False Then Return
	bmScale 	= rsip.createScaledBitmap(bm, bm.width, bm.height, True)
...
Sub sw_response_onConfirm()
	Starter.clsFunc.showLog($"${sw_response_onConfirm}"$, 0)
End Sub
...
Sub enableAlbumButton(enable As Boolean)
	Starter.clsFunc.showLog("ALBUM",0)
	If enable = True Then
...
Sub enableAlbumInfo(enable As Boolean)
	Starter.clsFunc.showLog(Starter.lyricFound, Colors.Red)
	Starter.clsFunc.showLog(Starter.vAlbumName, Colors.Red)
...
Sub setGenre(genre As String)
	Starter.clsFunc.showLog($"Genre : ${genre}"$, 0)
...
'Sub setPlayerStationLogo(bm As Bitmap)
'	'SET LOGO ON PANEL
'	Return
'	Dim panel As Panel
'	Dim ivLogo As Label
'	If Starter.vUpdateLogo = False Then
'		Return
'	End If
'	
'	
...
'	
'	For Each v As View In panel.GetAllViewsRecursive
'		If v.Tag = "stationLogo" Then
'			ivLogo = v
'			ivLogo.SetBackgroundImage(bm)
'			Exit
'		End If
'	Next
'End Sub
...
Sub ivTimer_Click
	...
	el = pnlNowPlaying.Elevation
	pnlNowPlaying.Elevation = 2dip
